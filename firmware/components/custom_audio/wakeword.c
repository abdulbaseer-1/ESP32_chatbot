...
void wakeword_init_multinet(wakeword_callback_t callback)
{
    ...
    ESP_LOGI(TAG, "WakeNet model initialized: %s", model_name);
}

void wakeword_task(void *arg)
{
    while (1) {
        int bytes_read = i2s_mic_read((char *)audio_buffer, audio_chunksize * sizeof(int16_t));
        if (bytes_read <= 0) {
            ESP_LOGW(TAG, "Failed to read I2S audio");
            continue;
        }
        ...
        if (state == WAKENET_DETECTED) {
            ...
            if (wakeword_callback) {
                wakeword_callback();  // Trigger user-defined callback
            }

            vTaskDelay(pdMS_TO_TICKS(1000)); // Optional delay to avoid duplicate triggers
        }
    }
}
